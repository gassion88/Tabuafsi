		...
		if( wp_verify_nonce( $_REQUEST['security'], 'classiera_nonce' ) ){
			...
			if(isset($_POST['main_comment_ID'])){
				...
				if(empty($commentData) || $commentData == ''){
					$html = '<p class="alert alert-warning">'.esc_html__( "You must type a comment!", "classiera" ).'</p>';
				}else{
					$post_author_id = '';
					$offer_author_id = '';
					$currentUserID = get_current_user_id();				
					...
					if(isset($tag_extra_fields[$cat_id]['category_custom_fields'])){
						...
						for ($i = 0; $i < (count($fields)); $i++) {
							...
							if($fieldsType[$i][1] == 'dropdown'){
								...
								echo '<div class="inner-addon right-addon">';
									echo '<i class="right-addon form-icon fas fa-sort"></i>';
									echo '<select name="Марка" class="form-control form-control-sm">';
										echo '<option value="">'.esc_attr($fields[$i][0]).'</option>';
										foreach($optionsarray as $option){
...
*/
				...
				if ($catID == "39") {
		                        ...
		                        echo '<div >';
									 echo '<input class="as_input" value="" type="text" name="Модель" placeholder="Модель" />';
								echo '</div></br>';			
		                        ...
		                        echo '<div >';
									 echo '<input class="as_input" value="" type="text" name="year_of_caк_ot" placeholder="Год выпуска от" />';
		                             echo'<input class="as_input" value="" type="text" name="year_of_caк_do" placeholder="Год выпуска до" />';
								echo '</div>';	
         ...
         <label for="engine_type5">гибрид</label></br>';
								...
								echo '<div class="inner-addon right-addon">';
									 echo '<input class="as_input" value="" type="text" name="engine_capacity_ot" placeholder="Объём двигателя от" />';
		                             echo'<input class="as_input" value="" type="text" name="engine_capacity_do" placeholder="Объём двигателя до" />';
								echo '</div>';

								echo '<div class="inner-addon right-addon">';
									echo '<i class="right-addon form-icon fas fa-sort"></i>';
									echo '<select name="steering" class="form-control form-control-sm">';
									echo '<option value="">Руль</option>';							
									echo '<option value="lub">любой</option>';
									echo '<option value="prav">правый</option>';
									echo '<option value="lev">левый</option>';
									echo '</select>';
								echo '</div>';
